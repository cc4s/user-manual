...
* Brief description
This algorithm calculates the structure factor for (ccsd) amplitudes provided.
CoulombVertexSingularVectors are needed to transform back to the common reciprocal 
mesh. Furthermore, a tricubic interpolaton can be performed which reduces
quadrature errors.
* Algorithm call
